...
/** Construct MovementCommandQueue object. Constructor creates and
...
/** Function manages publishing commands to the eDO robot.
 */
void MovementCommandQueue::sendMoveCommand(edo_core_msgs::msg::MovementCommand cmd){
  move_ctrl_pub->publish(cmd);
}

/** Function manages pushing commands onto the pendingQueue.
 *  If the executed and receive queues are both empty, this function pushes
 *  the resetCommand onto both queues.
 */
void MovementCommandQueue::pushMoveCommand(edo_core_msgs::msg::MovementCommand cmd){
...
/** Function check returns true if movement command queues have not been fully
 processed.
 */
