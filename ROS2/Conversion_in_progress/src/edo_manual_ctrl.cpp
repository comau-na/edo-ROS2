...
#include "edo_core_msgs/msg/MovementCommand.hpp"
#include "edo_core_msgs/msg/JointCalibration.hpp"
#include "edo_core_msgs/msg/JointReset.hpp"
#include "edo_core_msgs/msg/JointInit.hpp"
#include <thread>
#include <chrono>
#include <iostream>
#include <fstream>
...
//ros::Publisher jog_ctrl_pub =
  //  nh.advertise<edo_core_msgs::msg::MovementCommand>("/bridge_jog",10);

     rclcpp::Publisher<edo_core_msgs::msg::MovementCommand>::SharedPtr jog_ctrl_pub;

    jog_ctrl_pub = this->create_publisher<edo_core_msgs::msg::MovementCommand>("/bridge_move",
    10);
...
//    nh.advertise<edo_core_msgs::JointCalibration>("/bridge_jnt_calib",10);
  ...
  if(!recalib){
    ...
    */
    ...
    init_msg.mode = 0;
    init_msg.joints_mask = 127;//63 for without gripper;
   ...
   while(init_pub.getNumSubscribers() == 0){
    ...
    proceed = '\n';                             // Reset char for next prompt
    reset_msg.joints_mask = 127;//63 for without gripper;
